...
const Individualnsurancedetails = () => {
    ...
    const getIndividualContent = async () => {
        try {
            ...
            await fetch(`${API_URL}/api/getIndividualMedicalContent`, requestOptions)
                .then(response => response.json())
                .then(data => {
                    if (data.status === 200) {
                        setIndividualmedicalContent(data.data.individual_medical_Content);
                    } else {
                        swal("Error", "Error Fetching Content", "error");
                    }
                });
        } catch (error) {
            console.log(error);
    ...
    console.log(individual_medical_Content)
    ...
    return (
        <div>
            <div className='insurance_details pt-3 pb-3'>
                ...
                <div className='container' >
                    <div dangerouslySetInnerHTML={{ __html: individual_medical_Content }} />
                </div>
            </div>
        </div>
    )
...
export default Individualnsurancedetails;