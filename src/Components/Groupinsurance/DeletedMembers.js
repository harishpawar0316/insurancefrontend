...
const DeletedMembers = () => {
    ...
    const getLeads = async (page, limit) => {
        try {
            ...
            fetch(`${API_URL}/api/getHrUserLeads?limit=${limit}&page=${page}&leadType=deleted&planId=${customerid}&companyId=${insutancecompanyid}&tpaId=${tpaid}&networkId=${networkid}&policyNumber=${policyname}`, requestOptions)
                                        ...
                                        <tbody>
                                            {leads?.map((item, index) => (
                                                <tr>
                                                    ...
                                                    <td>
                                                        <button className='buttonblue1' onClick={() => Navigate(`/DeletedMembersView?id=${item?._id}`)}>
