...
const NewlyAddedMember = () => {
    ...
    const getLeads = async (page, limit) => {
        try {
           ...
           await fetch(`${API_URL}/api/getHrNewlyAddedMember?limit=${limit}&page=${page}&leadType=newlyAddedMember&planId=${customerid}&companyId=${insutancecompanyid}&tpaId=${tpaid}&networkId=${networkid}&policyNumber=${policyname}`, requestOptions)
                                    ...
                                    <Table striped bordered hover className='groupmedi'>
                                        ...
                                        <tbody>
                                            {leads.map((item, index) => (
                                                <tr>
                                                    <td>{item?.leadsData?.employeeNumber}</td>
                                                    <td>{`${item?.leadsData?.firstName} ${item?.leadsData?.middleName} ${item?.leadsData?.lastnName}`}</td>
                                                    <td>{item?.leadsData?.email}</td>
                                                    <td>{item?.leadsData?.phoneno}</td>
                                                    <td>{item?.leadsData?.AdditionEffectiveDate && item?.leadsData?.AdditionEffectiveDate != '' ? formatedate(item?.leadsData?.AdditionEffectiveDate) : "-"}</td>
                                                    <td>{item?.leadsData?.sentToJdvDate && item?.leadsData?.sentToJdvDate != '' ? formatedate(item?.leadsData?.sentToJdvDate) : "-"}</td>

                                                    <td>
                                                        <button className='buttonblue1' onClick={()=>Navigate(`/NewlyAddedMembersView?id=${item?.leadsData?._id}`)}>
...
export default NewlyAddedMember
